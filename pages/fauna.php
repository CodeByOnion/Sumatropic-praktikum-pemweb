...
<head>
    ...
    <style>
        /* Awal: kartu tidak terlihat dan turun sedikit */
        .fauna-card {
            opacity: 0;
        ...
        /* Akhir: kartu muncul (class ini ditambah via JS) */
        .fauna-card.muncul {
            opacity: 1;
...
<body>
    ...
    <script>
        ...
        if(navbar) {
            window.addEventListener('scroll', () => {
                if (window.scrollY > 50) {
                    ...
                    navbar.style.backgroundColor = ''; // Reset ke CSS asli
        ...
        // 2. JS ANIMASI SCROLL REVEAL (Agar kartu muncul satu-satu saat discroll)
            ...
            const observer = new IntersectionObserver((entries, observer) => {
                entries.forEach((entry, index) => {
                    if (entry.isIntersecting) {
                        // Tambah delay sedikit supaya munculnya estetik
                        setTimeout(() => {
