...
if (isset($_GET['hapus'])) {
    ...
    $q = mysqli_query($conn, "SELECT gambar FROM flora WHERE id='$id'");
    $data = mysqli_fetch_assoc($q);

    // Path gambar (Mundur satu folder karena file ini ada di actions/)
    $path = "../uploads/flora/" . $data['gambar'];
    if (file_exists($path) && $data['gambar']) {
        unlink($path);
    }

    // Hapus data DB
    mysqli_query($conn, "DELETE FROM flora WHERE id='$id'");

    // Redirect KEMBALI KE ADMIN
    echo "<script>alert('Data Flora Dihapus!'); window.location='../admin/kelola_flora.php';</script>";
    exit;
}
...
if (isset($_GET['id'])) {
    ...
    $query = mysqli_query($conn, "SELECT * FROM flora WHERE id='$id_flora'");
...
if (isset($_POST['simpan'])) {
    ...
    if ($_FILES['gambar']['name'] != "") {
        ...
        $tujuan = "../uploads/flora/" . $nama_baru;

        // Pastikan folder ada
        if (!is_dir("../uploads/flora/")) {
            mkdir("../uploads/flora/", 0777, true);
        }
        ...
        if (move_uploaded_file($tmp_file, $tujuan)) {
            ...
            if ($is_edit && file_exists("../uploads/flora/" . $gambar_lama)) {
                unlink("../uploads/flora/" . $gambar_lama);
            }
        }
    }
    ...
    if ($is_edit) {
        ...
        $query = "INSERT INTO flora (nama_lokal, nama_ilmiah, asal_provinsi, status_konservasi, deskripsi, gambar) 
                  VALUES ('$nama_lokal_in', '$nama_ilmiah_in', '$asal_in', '$status_in', '$deskripsi_in', '$gambar_db')";
    }

    if (mysqli_query($conn, $query)) {
        echo "<script>alert('Data Berhasil Disimpan!'); window.location='../admin/kelola_flora.php';</script>";
    } else {
...
<head>
    <meta charset="UTF-8">
    <title><?= $is_edit ? 'Edit' : 'Tambah'; ?> Flora</title>
...
<body>
    <aside class="sidebar">
        ...
        <a href="../admin/kelola_flora.php" style="background-color: #34495e;">Data Flora</a>
    ...
    <div class="container">
        <a href="../admin/kelola_flora.php">← Kembali ke Daftar Flora</a>
        <h2><?= $is_edit ? 'Edit Data Flora' : '+ Tambah Flora Baru'; ?></h2>
        ...
        <div class="form-box">
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <label>Gambar Flora</label>
                <?php if ($is_edit && $gambar_lama): ?>
                    <br><img src="../uploads/flora/<?= $gambar_lama ?>" class="preview">
